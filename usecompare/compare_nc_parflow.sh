...
# Paths to files
#Name_TEST=./execute/TSMP2_workflow-engine/run/sim_eclmparflow_20170701/eur-11u.out.00001.nc
#Name_TEST=/p/project1/cslts/shared_data/rcmod_TSMP2_WFE_reference_CORDEX_JURECA/simres/eclmparflow_20170701/out/parflow/eur-11u.out.00001.nc
#Name_REF=/p/project1/cslts/shared_data/rcmod_TSMP2_WFE_reference_CORDEX_JURECA/simres/eclmparflow_20170701/out/parflow/eur-11u.out.00001.nc
...
if [ ! -f "$Name_TEST" ]; then
    ...
    missing_files=1
    echo "The parflow comparison is MISSING-NC-files-TEST"
    exit 2
fi
if [ ! -f "$Name_REF" ]; then
    echo "ERROR: REF file does not exist: $Name_REF"
    missing_files=1
    echo "The parflow comparison is MISSING-NC-files-REF"
    exit 2	
fi
